...
class StarRail {
    ...
    getAllCharacters(playableOnly = true) {
        return new config_file_js_1.JsonReader(CachedAssetsManager_1.excelJsonOptions, this.cachedAssetsManager._getExcelData("AvatarConfig")).filterObject((_, c) => (playableOnly && c.getAsNumber("AdventurePlayerID") === c.getAsNumber("AvatarID")) || !playableOnly).map(([, c]) => new CharacterData_1.CharacterData(c.getAsNumber("AvatarID"), this));
    }
    /**
     * @returns all light cone data
     */
    getAllLightCones(excludeTestLightCones = true) {
        return new config_file_js_1.JsonReader(CachedAssetsManager_1.excelJsonOptions, this.cachedAssetsManager._getExcelData("EquipmentConfig")).filterObject((_, lc) => (excludeTestLightCones && lc.has("AvatarBaseType")) || !excludeTestLightCones).map(([, lc]) => new LightConeData_1.LightConeData(lc.getAsNumber("EquipmentID"), this));
    }
    /**
     * @returns all relic data
     */
    getAllRelics() {
        return new config_file_js_1.JsonReader(CachedAssetsManager_1.excelJsonOptions, this.cachedAssetsManager._getExcelData("RelicConfig")).mapObject((_, relic) => new RelicData_1.RelicData(relic.getAsNumber("ID"), this));
    }
    /**
