...
class Character {
    constructor(data, client) {
        ...
        this.skillTreeNodes = json.get("skillTreeList").mapArray((_, skill) => {
            ...
            const levelUpByEidolons = node.levelUpSkills.length > 0 ? unlockedEidolons.reduce((levels, eidolon) => levels + (eidolon.skillsLevelUp[node.levelUpSkills[0].id]?.levelUp ?? 0), 0) : 0;
        ...
        this.skills = this.characterData.skills.map(skill => {
            ...
            const level = skillNode?.level ?? new SkillLevel_1.SkillLevel(1, 0);
        ...
        if (this.characterData.maxEnergy !== null) {
            this.basicStats.push(new StatProperty_1.StatPropertyValue("MaxSP", this.characterData.maxEnergy, this.client));
        }
