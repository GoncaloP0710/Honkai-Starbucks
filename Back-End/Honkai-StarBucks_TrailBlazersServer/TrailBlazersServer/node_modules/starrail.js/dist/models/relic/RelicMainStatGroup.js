"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.RelicMainStatGroup = void 0;
const config_file_js_1 = require("config_file.js");
const AssetsNotFoundError_1 = require("../../errors/AssetsNotFoundError");
const StatProperty_1 = require("../StatProperty");
const CachedAssetsManager_1 = require("../../client/CachedAssetsManager");
class RelicMainStatGroup {
    constructor(id, client) {
        this.id = id;
        this.client = client;
        const _data = client.cachedAssetsManager.getExcelData("RelicMainAffixConfig", this.id);
        if (!_data)
            throw new AssetsNotFoundError_1.AssetsNotFoundError("RelicMainStatGroup", this.id);
        this._data = _data;
        const json = new config_file_js_1.JsonReader(CachedAssetsManager_1.excelJsonOptions, this._data);
        this.mainStats = json.mapObject((_, v) => {
            return {
                id: v.getAsNumber("AffixID"),
                groupId: this.id,
                statProperty: new StatProperty_1.StatProperty(v.getAsString("Property"), this.client),
                baseValue: v.getAsNumber("BaseValue", "Value"),
                levelValue: v.getAsNumber("LevelAdd", "Value"),
            };
        });
    }
}
exports.RelicMainStatGroup = RelicMainStatGroup;
