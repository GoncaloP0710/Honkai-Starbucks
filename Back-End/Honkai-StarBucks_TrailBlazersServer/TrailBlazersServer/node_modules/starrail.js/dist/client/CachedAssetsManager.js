...
exports.CachedAssetsManager = exports.excels = exports.excelKeyMap = exports.excelJsonOptions = void 0;
...
let excelDataMemory = { ...initialExcelDataMemory };
...
let langDataMemory = { ...initialLangDataMemory };
...
class CachedAssetsManager {
    ...
    async cacheDirectorySetup() {
        ...
        if (!fs_1.default.existsSync(githubCachePath) || !this._githubCache) {
            this._githubCache = await new config_file_js_1.ConfigFile(githubCachePath, { allowBigint: false }, {
                "lastUpdate": 0,
                "rawLastUpdate": 0,
            }).load();
        }
    ...
    async checkForUpdates(useRawStarRailData = false) {
        await this.cacheDirectorySetup();
        const url = getGitRemoteAPIUrl(useRawStarRailData, new Date(this._githubCache?.getValue("rawLastUpdate") ?? 0), new Date(this._githubCache?.getValue("lastUpdate") ?? 0));
    ...
    async fetchAllContents(options) {
        ...
        if (!options.useRawStarRailData) {
            ...
            const promises = [];
            const excelOutputData = { ...initialExcelDataMemory };
            for (const excel of exports.excels) {
                const fileName = `${excel}.json`;
                const url = `${excelBaseUrl}/ExcelOutput/${fileName}`;
                promises.push((async () => {
                    const json = (await (0, axios_utils_1.fetchJSON)(url, this.client, false, true)).data;
                    if (this.client.options.showFetchCacheLog) {
            ...
            const langsData = { ...initialLangDataMemory };
            const langPromises = [];
            ...
            for (const excel of Object.keys(excelOutputData)) {
                fs_1.default.writeFileSync(path_1.default.resolve(this.cacheDirectoryPath, "data", `${excel}.json`), JSONBig.stringify(excelOutputData[excel]));
            }
    ...
    async updateContents(options = {}) {
        ...
        await this.cacheDirectorySetup();
        const url = getGitRemoteAPIUrl(!!options.useRawStarRailData, new Date((this._githubCache?.getValue("rawLastUpdate") ?? 0)), new Date((this._githubCache?.getValue("lastUpdate") ?? 0)));
        const res = await (0, axios_utils_1.fetchJSON)(url, this.client);
        if (res.status !== 200) {
            throw new Error("Request Failed");
        }
     ...
     */
    _getExcelData(excel) {
        excelDataMemory[excel] ?? (excelDataMemory[excel] = JSONBig.parse(fs_1.default.readFileSync(this.getJSONDataPath(excel), "utf-8")));
    ...
    getLanguageData(lang) {
        langDataMemory[lang] ?? (langDataMemory[lang] = JSON.parse(fs_1.default.readFileSync(this.getLanguageDataPath(lang), "utf-8")));
    ...
    removeUnusedTextData(data, langsData, showLog = true) {
        ...
        function push(...keys) {
            ...
            for (let i = 0; i < len; i++) {
                ...
                if (!requiredKeys.has(keyStr))
                    requiredKeys.add(keyStr);
            }
        }
        push(...textMapWhiteList);
        Object.values(data["AvatarConfig"]).forEach(c => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, c);
            push(json.getAsNumberOrBigint("AvatarName", "Hash"));
        });
        Object.values(data["ItemConfigAvatar"]).forEach(c => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, c);
            push(json.getAsNumberOrBigint("ItemBGDesc", "Hash"));
        });
        Object.values(data["DamageType"]).forEach(d => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, d);
            push(json.getAsNumberOrBigint("DamageTypeName", "Hash"), json.getAsNumberOrBigint("DamageTypeIntro", "Hash"));
        });
        Object.values(data["AvatarBaseType"]).forEach(p => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, p);
            push(json.getAsNumberOrBigint("BaseTypeText", "Hash"), json.getAsNumberOrBigint("BaseTypeDesc", "Hash"));
        });
        Object.values(data["AvatarSkillConfig"]).forEach(s => {
            Object.values(s).forEach(l => {
                const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, l);
                push(json.getAsNumberOrBigint("SkillName", "Hash"), json.getAsNumberOrBigint("SkillTag", "Hash"), json.getAsNumberOrBigint("SkillTypeDesc", "Hash"), json.getAsNumberOrBigintWithDefault(null, "SkillDesc", "Hash"), json.getAsNumberOrBigintWithDefault(null, "SimpleSkillDesc", "Hash"));
            });
        });
        ...
        Object.values(data["AvatarRankConfig"]).forEach(e => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, e);
        ...
        Object.values(data["AvatarSkin"]).forEach(s => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, s);
            push(json.getAsNumberOrBigint("AvatarSkinName", "Hash"));
        });
        Object.values(data["EquipmentConfig"]).forEach(l => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, l);
            push(json.getAsNumberOrBigint("EquipmentName", "Hash"), json.getAsNumberOrBigintWithDefault(null, "EquipmentDesc", "Hash"));
        });
        Object.values(data["ItemConfigEquipment"]).forEach(l => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, l);
            push(json.getAsNumberOrBigint("ItemBGDesc", "Hash"), json.getAsNumberOrBigint("ItemDesc", "Hash"));
        });
        Object.values(data["EquipmentSkillConfig"]).forEach(s => {
            Object.values(s).forEach(l => {
                const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, l);
                push(json.getAsNumberOrBigint("SkillName", "Hash"), json.getAsNumberOrBigint("SkillDesc", "Hash"));
            });
        });
        Object.values(data["ItemConfigRelic"]).forEach(r => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, r);
            push(json.getAsNumberOrBigint("ItemName", "Hash"), json.getAsNumberOrBigint("ItemBGDesc", "Hash"));
        });
        Object.values(data["RelicSetConfig"]).forEach(s => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, s);
            push(json.getAsNumberOrBigint("SetName", "Hash"));
        });
        Object.values(data["RelicSetSkillConfig"]).forEach(s => {
            Object.values(s).forEach(b => {
                const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, b);
                push((0, hash_utils_1.getStableHash)(json.getAsString("SkillDesc")));
            });
        });
        Object.values(data["RelicBaseType"]).forEach(t => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, t);
            push(json.getAsNumberOrBigintWithDefault(null, "BaseTypeText", "Hash"));
        });
        Object.values(data["AvatarPropertyConfig"]).forEach(s => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, s);
            push(json.getAsNumberOrBigint("PropertyName", "Hash"), json.getAsNumberOrBigintWithDefault(null, "PropertyNameSkillTree", "Hash"), json.getAsNumberOrBigint("PropertyNameRelic", "Hash"), json.getAsNumberOrBigint("PropertyNameFilter", "Hash"));
        });
        [...Object.values(data["ItemConfigAvatarPlayerIcon"]), ...Object.values(data["ItemPlayerCard"])].forEach(i => {
            const json = new config_file_js_1.JsonReader(exports.excelJsonOptions, i);
            push(json.getAsNumberOrBigint("ItemName", "Hash"));
        });
        if (showLog)
            console.info(`Required keys have been loaded (${requiredKeys.size.toLocaleString()} keys)`);
        const clearLangsData = { ...initialLangDataMemory };
